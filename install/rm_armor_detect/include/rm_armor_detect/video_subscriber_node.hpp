...
#include "rm_armor_detect/pnp_solver.hpp"
...
private:
  ...
  // PnP解算相关函数
  void init_pnp_solver();
  void process_armor_pnp(const std::vector<cv::Point2f> & armor_points, ArmorType armor_type, cv::Mat & image);
  void draw_pnp_result(cv::Mat & image, const PnPResult & result, const cv::Point2f & armor_center);
  std::vector<cv::Point2f> extract_armor_points_traditional(const Armor & armor);
  std::vector<cv::Point2f> extract_armor_points_neural(const std::vector<cv::Point2f> & keypoints);
  
  // 装甲板发布相关函数
  void publish_armor_detections(const std::vector<Armor> & armors, const std_msgs::msg::Header & header, const std::vector<PnPResult> & pnp_results = {});
  void publish_armor_detections_neural(const std::vector<ArmorDetection> & detections, const std_msgs::msg::Header & header, const std::vector<PnPResult> & pnp_results = {});
  void load_armor_names();
  std::string get_armor_name_by_number(const std::string & number) const;
  bool is_enemy_color(Color armor_color) const;
  ...
  // 装甲板发布者
  rclcpp::Publisher<rm_msgs::msg::Armor>::SharedPtr armor_publisher_;
  
  // 装甲板名称映射
  std::unordered_map<std::string, std::string> armor_name_map_;
